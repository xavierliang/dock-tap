...
APP="$ROOT/build/DockTapProbe.app"
CONTENTS="$APP/Contents"
MACOS="$CONTENTS/MacOS"
RESOURCES="$CONTENTS/Resources"
BUNDLE_ID="dev.local.DockTapProbe"
...
cp "$BIN_DIR/DockTapProbe" "$MACOS/DockTapProbe"
cp "$ROOT/Resources/Info.plist" "$CONTENTS/Info.plist"
chmod +x "$MACOS/DockTapProbe"
...
if [[ -n "$CODESIGN_IDENTITY" ]]; then
    ...
    echo "WARNING: USING UNSTABLE AD-HOC SIGNING. ACCESSIBILITY/TCC AUTHORIZATION MAY BE LOST ON EVERY REBUILD."
    echo "WARNING: THIS IS ONLY FOR LAUNCH SMOKE TESTS, NOT FOR M0 MANUAL TCC VALIDATION."
...
Set DOCK_TAP_CODESIGN_IDENTITY to a local code signing certificate name, for example:
  DOCK_TAP_CODESIGN_IDENTITY="Apple Development: ..." scripts/run-probe.sh
...
For a launch-only smoke test, opt in explicitly:
  DOCK_TAP_ALLOW_UNSTABLE_ADHOC=1 scripts/run-probe.sh
EOF
    exit 1
fi
